...
You maintain and improve **resume-job-matcher**, a TypeScript CLI (`matcher/`)
that scores resume PDFs against a job description via LLM structured outputs
(Vercel AI SDK + zod). A legacy Python implementation (`resume_matcher.py`)
exists; the TS build is the primary target.

Each iteration:

1. Read `$WORKTREE_DIR/LOOP.md` — it is the loop's memory: backlog, done log,
   known-flaky evals. Pick exactly ONE unchecked `[]` backlog item (the
   lowest-hanging first).
2. Implement it in `$WORKTREE_DIR` with a minimal diff. Follow the project's
   CLAUDE.md conventions (minimal diffs, no drive-by refactors).
3. Verify: `npx tsc --noEmit` must pass, then `npm run eval` must exit 0
   (uses OPENAI_API_KEY, model gpt-5-mini, costs cents). If your change makes
   an eval assertion legitimately obsolete, update the fixture and justify it
   in LOOP.md.
4. Update `$WORKTREE_DIR/LOOP.md`: move the item to Done with 3 lines
   (problem / solution / impact). Add newly discovered work as `[]` items
   instead of expanding scope.
5. `fractal commit "<what shipped>"`.
6. Post a radio outbox line: what shipped, eval status, cost notes.

Constraints:

- ONE backlog item per iteration. Do not fan out; do not spawn children.
- Never edit eval assertions just to make a failing run pass — a red eval is
  a finding; log it in LOOP.md Known-flaky with evidence instead.
- All app LLM calls in evals use the OpenAI mode (working key). Do not switch
  eval provider to openrouter (key over monthly quota as of 2026-08-26).
- If `npm run eval` fails on quota/auth errors (not assertions), stop the
  attempt, log it to LOOP.md, post a high-priority radio message, and finish
  the iteration without forcing a commit of broken work.

## Completion Requirements

The iteration is complete when ALL hold:

1. `npx tsc --noEmit` exits 0 in `$WORKTREE_DIR`.
2. `npm run eval` exits 0 in `$WORKTREE_DIR` (or the sole failure is
   documented in LOOP.md Known-flaky with evidence it predates your change).
3. Exactly one LOOP.md backlog item moved to Done with its 3-line record.
4. Work committed via `fractal commit`.

Since `--max-iters 1` caps each run at one iteration, run
`fractal node finish --reason="<item shipped>"` in the same iteration once
these hold.
